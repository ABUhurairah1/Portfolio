...
(function ($) {
  ...
  const oneNavOnePage = () => {
    ...
    $navLinks.on("click", function (e) {
      ...
      if ($(this).hasClass("open-popup")) {
        // openYourPopup(); // Function not defined - comment out or define elsewhere
      }
    });
  ...
  -------------------------------------------------------------------------*/
  const spliting = () => {
    if ($(".splitting").length && typeof Splitting !== 'undefined') {
      Splitting();
    }
  };
  ...
  const handleSidebar = () => {
    ...
    $(document)
      ...
      .on("click.handleSidebar", ".show-sidebar", function (e) {
        ...
        // Kiểm tra nếu không còn popup nào mở thì remove no-scroll
        if (
